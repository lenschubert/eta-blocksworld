...
;;  (format t "~%Schema to be used for plan ~a, with arguments instantiated~
 ;;            ~% ~a" plan-name plan); DEBUGGING
  ...
  ; If this is a Lissa action, transfer to it the gist clauses, interpretation,
 ...
 (let* ({sub}plan-name (rest (get plan-name 'rest-of-plan)) wff)
       ...
       (cond ; First we try to match '(me ...)' (Lissa) actions, typically
           ...
           ((eq (car wff) 'me) ; Lissa action
            (progn (implement-next-lissa-action {sub}plan-name) (print ""))) 
                   ; also increments *count* for primitive lissa actions
...
(defun remove-contradiction (gist-list) ; added post-2015 (& tidied up 2019)
;``````````````````````````````````````
; removes any contradiction if exists. If two statements contradicts, it
; removes the latter one, which is the one appeared later in the user's input.
  (cond ((<= (list-length gist-list) 1) gist-clause)
        (T
         (if (gist-contradiction (butlast gist-list) (car (last gist-list)))
             (butlast gist-list); don't add the gist 
             (append (remove-contradiction (butlast gist-list)) 
                     (last gist-list))))
 )); end of remove-contradiction
