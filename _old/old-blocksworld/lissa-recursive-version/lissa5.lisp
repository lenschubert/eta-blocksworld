...
(defun purify_func1 (user-gist-clauses); a post-2015 addition
;``````````````````````````````````````
; Remove user gist clauses identical with '(NIL GIST), except such an item
; in last place, when preceded by at least one non-(NIL GIST) item;.
(if (null user-gist-clauses) 
    nil 
...
;
 (let (wordstring)
	  ...
	  ; added for the elderly study. Unlike *lissa-count* we do not reset this variable 
	  ; at every topic but we reset it at the beginning of every day
	  ...
	  ; this is for elderly study where we have to wait for the completion of the last utterence 
	  ; until 
	  (if (search "LETS PAUSE HERE SO I CAN GIVE" wordstring) 
	      (sleep 5))
	  ;(with-open-file (lockfile "./lock" :if-does-not-exist :create)
	  
	  ; if the sentence contains "lets pause here", wait for a second and then print it. 
	  ...
	  (with-open-file (outfile "./output.txt" :direction :output :if-exists
                                :append :if-does-not-exist :create)
                      (format outfile "~%#~D: ~a" *global-count* wordstring));)   ; used to be lissa-count, changed it for elderly study
      (setf *default-pathname-defaults* (truename *temp-dir*))
     ;(delete-file "./lock")
...
;````````````````````
 ...
 ; property of the word, from the FEATS property iof the 
...
(defun add-new-feats (feat old-feats-list words)
  (setq new-feats-list old-feats-list)
;  (print "hi")
...
;  (print "hi2")
  ...
  (return-from add-new-feats new-feats-list)   ; for members of the above list 
