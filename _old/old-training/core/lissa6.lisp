 ...
 (let (user-gist-words choice tagged-words wff subplan-name 
      ...
      (cond ((null (cdr user-gist-clauses)); single clause?
            ...
            (T ; there are multiple gist clauses;
               ...
               ; here:  if there exist multiple gist clasuse, remove those 
               ; with "nothing found"
      ...
      (cond ((eq (car choice) :out); a verbal reaction
            ...
            ; anywhere, so I added it. -Philip Meyers
            ((eq (car choice) :schema)
             (setq schema-name (first (cdr choice)))
             (set subplan-name (initialize-plan subplan-name schema-name nil)))
...
(defun purify_func1 (user-gist-clauses); a post-2015 addition (Philip Meyers?)
;``````````````````````````````````````
...
; in last place, when preceded by at least one non-(NIL GIST) item; (Why? -LS)
 (if (null user-gist-clauses) 
     nil 
     (if (and (eq (caar user-gist-clauses) 'NIL) (eq (cadar user-gist-clauses) 'Gist)) 
...
(defun purify_func (x)
;````````````````````
 (if (and (eq (length x) 1) (eq (caar x) 'NIL) (eq (cadar x) 'Gist))
...
(defun choose-result-for1 (tagged-clause parts rule-node); Aug 14-26/15
  ...
  (prog (directive pattern newparts newclause new-tagged-clause ulf
         ulfs result) 
    ...
    ;(format t tagged-clause)	
     ;   (format t "~% =====2==== Pattern/output to be matched in rule ~a = ~
      ;         ~%  ~a and directive = ~a" rule-node pattern directive); DEBUGGING
        (cond ; First we deal with the cases requiring further tree-descent
              ; (with possible failure and thus recursive backtracking);
              ;
              ; no directive (i.e., decomposition rule), :subtree, :subtree+clause,
              ; and :ulf-recur;
            ...
            ;;                      rule-node pattern tagged-clause parts))
               (if (null newparts); pattern does not match 'tagged-clause'
                   (return ; search siblings (alternatives) recursively;
                           ; (if there are no further siblings, then we're
                           ; out of options and will fail back to the calling
                           ; program instance); 
              ...
              ((eq directive :ulf-recur) 
               ...
               ; phrasal ULFs.
               (setq newclause
                  (instance (first pattern) parts)); instantiate shallow analysis;
               (dolist (phrase newclause); interpret recursive phrases; the car
                  ; of each nonatomic phrase either gives the name of the relevant
                  ; rule tree to use, or it is 'lex-ulf!'; in the former case we
                  ; proceed recursively, in the latter we keep the phrase as-is:
                  (setq ulf 
                        (if (or (atom phrase) (eq (car phrase) 'lex-ulf!))
                            phrase ; e.g., ulf of (next to) = next_to.p
                            (setq ulf (choose-result-for 
                                  (mapcar #'tagword (cdr phrase)) (car phrase)))))
                  (when (null ulf) ; (failure)            rule name``````````
                        (setq ulfs nil)
                        (return nil)); exit loop
                  (push ulf ulfs))
               ; Assemble the (initially reversed) list of phrasal ULFs into
               ; a ULF for the entire input, using the second reassembly rule:
               (when ulfs
                  (setq n (length ulfs)); the number of phrasal ULFs
                  (setq result (second pattern)); bracket structure, with indices
                  (dolist (ulf ulfs); phrasal ULFs are in reverse order here
                     (setq ulf (eval-lexical-ulfs ulf))
                     (setq result (subst ulf n result))
                     (decf n)))
               (return result)); note: no prefixing of result with ':ulf-recur'
...
(defun form-ulf-from-clause (clause); June 10/19
;````````````````````````````````````````````````
; Find the ULF corresponding to the user's 'clause' (a gist clause).
; **Right now, this uses *yn-question-ulf-tree* directly, instead of
...
;   it should replace *yn-question-ulf-tree* below.
;
...
;
 (let (tagged-clause)
      ...
      (choose-result-for tagged-clause '*yn-question-ulf-tree*)
...
;
 (let (wordstring)
      ...
      ; this is for elderly study where we have to wait for the completion
      ; of the last utterence until 
      (if (search "LETS PAUSE HERE SO I CAN GIVE" wordstring) 
	      (sleep 5))
      ;(with-open-file (lockfile "./lock" :if-does-not-exist :create)
      ; if the sentence contains "lets pause here", wait for a second and 
      ; then print it. 
...
(DEFUN RESET-RULE (RULE) ; Reset TIME-LAST-USED to -100 in all
...
;	     (format t "membership = ~a ~%" (member feat feats))
	     (if (not (find feat words))
                 (push feat words))
;             (format t "words = ~a ~%" words)  
             (setq feats (add-new-feats feat feats words)); add feats of
;             (format t "updated feature vector = ~a~%" feats) ; current word
...
(defun add-new-feats (feat old-feats-list words); added post-2015 (tidied 2019)
;```````````````````````````````````````````````
 (let ((new-feats-list old-feats-list) feat-feature-list)
;  (print "hi")
      (setq feat-feature-list (get feat 'feats))
;  (print "hi2")
      (loop for x in feat-feature-list 
	do (if (not (member x words)) 
	       (setq new-feats-list (append (list x) new-feats-list))))
      new-feats-list ; for members of the above list 
     ;if member m is not already in words list 
 )); end of add-new-feats
...
(defun instance (pattern parts); written June 11/19
;`````````````````````````````
...
;
 (let ((n (length parts)) part)
      (cond ((and (integerp pattern) (> pattern 0) (<= pattern n))
...
(defun instance1 (pattern parts); June 11/19
;``````````````````````````````
...
;
 (if (atom pattern) 
     ...
     (let ((n (length parts)))
          (apply #'append
            (mapcar #'(lambda (x) (if (and (integerp x) (> x 0) (<= x n))
...
(defun gist-contradiction (current-gist-list gist-clause); added post-2015
;````````````````````````````````````````````````````````
  (let (cont-flag); inserted by LS -- previously was global (though only used here)
       (loop for ix from 1 to (list-length current-gist-list)
             do
             (if (equal (car (or (set-difference 
                                   (nth (- ix 1) current-gist-list) gist-clause)
                                 (set-difference gist-clause 
                                   (nth (- ix 1) current-gist-list)))) 'NOT) 
                 (setf cont-flag T))
       ); end of loop
       cont-flag
 )); end of gist-contradiction
...
(defun gist-contradiction (current-gist-list gist-clause)
        ...
        (loop for ix from 1 to (list-length current-gist-list)
            do
            (if (equal (car (or (set-difference (nth (- ix 1) current-gist-list) gist-clause) (set-difference gist-clause (nth (- ix 1) current-gist-list)))) 'NOT) (setf cont-flag T))
        )
(return-from gist-contradiction cont-flag))

(defun remove-contradiction (gist-list)     ; removes any contradiction if exists. If two statements contradicts, 
                                            ; it removes the latter one, which is the one appeared later in the user's input.
        (cond ((<= (list-length gist-list) 1)
               (return-from remove-contradiction  gist-list))
              (T
               (if (gist-contradiction (butlast gist-list) (car (last gist-list)))
                   (return-from remove-contradiction (butlast gist-list))  ; do not add the gist 
                   (return-from remove-contradiction (append (remove-contradiction (butlast gist-list)) (last gist-list)))))
         )
)
