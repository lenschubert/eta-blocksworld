;; May 29/19
;; Noun and verb stemming code borrowed form extract-patterns.lisp in
;; /u/schubert/knowledge-mining-from-text/my-programs/VERSION-FOR-SHARING/
;;
;; The noun stemming doesn't deal with Latin or Greek plurals.
;; "People", "cattle", etc., get stemmed as "person", "cow", ..., but this may
;; be inappropriate for present purposes. Maybe we should use "people*", 
;; or perhaps "1people", etc.? (In ULF, though, we do use (plur person.n).)
;; ========================================================================
;;
;; Note: Some of the 'isa'-flaging below isn't needed for stemming, but
;; was carried over from extract-patterns.lisp.
;;
...
;;     (\-RRB\- \-RRB\-), etc.; these symbols are accepted as parts of speech
;;     by default, not being listed as such. (This is ok inasmuch as punctuation
...
;; CD is here generalized to DET, although A would be semantically more accurate;
;; I initially used A, but since Treebank makes no distinction between NP-initial
;; numeral adjectives and NP-internal ones, this proved more problematic than
;; using DET as generalization. (The trick we can use, for approximate
;; interpretation, is to ignore everything prior to a CD.) -- Jun 17/02
...
(setf (get 'DETP 'isa) 'DET)
(mapcar #'(lambda (x) (setf (get x 'isa) 'AUX))
   '(AUX AUXD AUXG AUXEN AUXP AUXZ) ); no AUXN -- aux's don't have passive forms
(mapcar #'(lambda (x) (setf (get x 'isa) 'BE))
...
(defun stem (pos-word) ; Mar 21/01
;~~~~~~~~~~~~~~~~~~~~~
; CRUDE -- this does only some very simple-minded stemming!
               ...
               (progn
                  ...
                  (setq result
                        (case word
                              ...
                              ((have had has having \'ve hath haveth) 'HAVE) 
                              ...
                              ((signalling signalled signaling signaled) 'SIGNAL)
                              ((sheathe sheathed sheathes sheathing) 'SHEATHE)
                              ((speak spoke speaks spoken speaking) 'SPEAK)
                              ((sank sunk sunken) 'SINK)
                              ((signalling signalled signaling signaled) 'SIGNAL)
                              ((wreathe wreathed wreathes wreathing) 'WREATHE)
